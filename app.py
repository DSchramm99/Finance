...
if page == "Signals":
    ...
    if st.sidebar.button("🚀 Generate Top 5 Signals"):
        ...
        # Performance Optimization: Parallel ticker analysis using ThreadPoolExecutor
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_ticker = {executor.submit(worker, ticker): ticker for ticker in tickers}
    ...
    if "results" in st.session_state:
        ...
        # Efficient company name lookup using centralized helper
...
if page in ["Test", "Live"]:
    ...
    if not open_trades_df.empty:
        ...
        for _, trade in open_trades_df.iterrows():
            ticker = trade["ticker"]
            try:
                # Optimized name retrieval from centralized metadata helper
