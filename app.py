...
if page == "Signals":
    ...
    if st.sidebar.button("🚀 Generate Top 5 Signals"):
        ...
        def analyze_ticker_with_ctx(ticker, lev_mode, context):
            # Apply Streamlit context to the current thread for @st.cache_data to work correctly
            add_script_run_ctx(threading.current_thread(), context)
            return analyze_ticker(ticker, lev_mode)

        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(analyze_ticker_with_ctx, ticker, is_leveraged, ctx) for ticker in tickers]
            ...
            for i, future in enumerate(as_completed(futures)):
                ...
                status_text.text(f"Analyse läuft... ({i+1}/{len(tickers)})")
